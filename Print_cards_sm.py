...
def main():
    ...
    try:
        ...
        for card in root.find("fronts").findall("card"):
            ...
            filename_with_id = f"{card_name.rsplit('.', 1)[0]} ({card_id}).{card_name.rsplit('.', 1)[1]}"
            image_path = images_dir / filename_with_id

            # Generate cropped images for each copy
            if image_path.exists():
                for copy_number in range(copies):
                    output_image_path = output_folder / f"{filename_with_id.rsplit('.', 1)[0]}_copy_{copy_number + 1}.bmp"
